...
cdef class CrossArbitrageStrategy(StrategyBase):
    ...
    def init_params(self,
        ...
        """
        :param market_pairs: list of cross_arbitrage market pairs
        :param min_profitability: minimum profitability limit, for calculating cross_arbitrage order sizes
    ...
    cdef c_tick(self, double timestamp):
        ...
        For cross_arbitrage strategy, this function simply checks for the readiness and connection status of markets, and
        ...
        try:
            if not self._all_markets_ready:
                ...
                if not self._all_markets_ready:
                    ...
                    if should_report_warnings:
                        self.logger().warning(f"Markets are not ready. No cross_arbitrage trading is permitted.")
                    return
                else:
            ...
            if not all([market.network_status is NetworkStatus.CONNECTED for market in self._sb_markets]):
                if should_report_warnings:
                    self.logger().warning(f"Markets are not all online. No cross_arbitrage trading is permitted.")
                return
    ...
    cdef tuple c_calculate_cross_arbitrage_top_order_profitability(self, object market_pair):
        """
    ...
    cdef bint c_ready_for_new_orders(self, list market_trading_pair_tuples):
        """
        Check whether we are ready for making new cross_arbitrage orders or not. Conditions where we should not make further
        new orders include:
        ...
        :param market_trading_pair_tuples: list of cross_arbitrage market pairs
        ...
        if self._cool_off_logged:
            self.log_with_clock(
                logging.INFO,
                f"Cool off completed. CrossArbitrage strategy is now ready for new orders."
            )
    ...
    cdef c_process_market_pair(self, object market_pair):
        ...
        :param market_pair: cross_arbitrage market pair
        """
        if not self.c_ready_for_new_orders([market_pair.first, market_pair.second]):
            return

        self._current_profitability = \
            self.c_calculate_cross_arbitrage_top_order_profitability(market_pair)
    ...
    cdef c_process_market_pair_inner(self, object buy_market_trading_pair_tuple, object sell_market_trading_pair_tuple):
        """
        Executes cross_arbitrage trades for the input market pair.
    ...
    @staticmethod
    def find_profitable_cross_arbitrage_orders(min_profitability: Decimal,
        ...
        return c_find_profitable_cross_arbitrage_orders(min_profitability,
    ...
    cdef tuple c_find_best_profitable_amount(self, object buy_market_trading_pair_tuple, object sell_market_trading_pair_tuple):
        ...
        profitable_orders = c_find_profitable_cross_arbitrage_orders(self._min_profitability,
...
cdef list c_find_profitable_cross_arbitrage_orders(object min_profitability,
